...
const ImageUploader = () => {
  ...
  const fileExtensionRemove = /\.[a-zA-Z]{3,4}$/;

  const uploadImage = (e) => {
    const files = e.target?.files;
    if (!files) return;
    let url = URL.createObjectURL(files[0]);
    let name = files[0].name; // Get the file name
    // Check if the image with the same name is already uploaded
    if (images.some((img) => img.name === name)) {
      return;
    }
    // Store the url and name as an object
    setImages((prev) => [...prev, { url, name }]);
  };

  const deleteImg = (name) => {
    // Compare the names to remove the image
          ...
          {!!images?.length &&
            images.map((img) => (
              ...
              >
                <Image
                  src={img.url}
                  alt={img.name.replace(fileExtensionRemove, "")}
